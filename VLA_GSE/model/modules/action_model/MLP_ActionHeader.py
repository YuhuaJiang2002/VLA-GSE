...
class L1RegressionActionHead(nn.Module):
    ...
    def __init__(
        ...
        action_dim=7,
        NUM_ACTIONS_CHUNK=8,
        num_blocks=2,  # Number of MLP ResNet blocks (layers)
    ):
        super().__init__()
        ...
        self.model = MLPResNet(
            num_blocks=num_blocks, input_dim=input_dim , hidden_dim=hidden_dim, output_dim=action_dim
        )
...
def get_action_model(config=None):
    ...
    Configurable parameters in config.framework.action_model:
        - action_hidden_dim: Input dimension (auto-set from VLM hidden_size)
        - action_dim: Output action dimension (default: 7)
        - future_action_window_size: Number of future actions to predict
        - past_action_window_size: Number of past actions
        - num_blocks: Number of MLP ResNet blocks/layers (default: 2)
        - hidden_dim_multiplier: Hidden dim = input_dim * multiplier (default: 2)
    """
    ...
    # Configurable number of layers and hidden dimension
    num_blocks = action_model_cfg.get("num_blocks", 2)
    hidden_dim_multiplier = action_model_cfg.get("hidden_dim_multiplier", 2)

    action_model = L1RegressionActionHead(
        input_dim=action_hidden_dim,
        hidden_dim=action_hidden_dim * hidden_dim_multiplier,
