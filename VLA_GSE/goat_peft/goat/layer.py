...
class TopKGOATLayer(nn.Module):
    """Top-K gated MoE layer for GOAT"""
    ...
    def get_expert_similarity_loss(self):
        ...
        lora_A_score = lora_A_flatten @ lora_A_flatten.T / (norm_A @ norm_A.T)
...
class GOATLayer(BaseTunerLayer, ABC):
    ...
    def update_layer(
        ...
        num_experts: int, top_k: int, init_type: str, init_cof: float = None,
        scaling_factor: int = None,         
    ) -> None:
        """Update the GOAT layer with new adapter"""
        if lora_rank <= 0:
        ...
        need_svd = "goat" in init_type or "svd" in init_type
...
class LinearGOATLayer(nn.Module, GOATLayer):
    """Linear layer with GOAT adaptation"""
