...
"""
VLA-GSE LoRA trainer.
Based on train_vla_gse.py, but uses LoRA for the VLM backbone while keeping the action head fully trainable.

Key Features:
  - Load pretrained base_vlm
  - Apply LoRA to the VLM backbone (freeze non-LoRA VLM parameters)
  - Keep action_model fully trainable (not using LoRA)
  - Finetune with LoRA + action head for efficient adaptation
"""
...
def apply_lora_to_vlm(model, cfg):
    """
    Apply LoRA to the VLM backbone while keeping action_model fully trainable.
    ...
    if vlm_model is None:
        logger.warning("Could not find VLM model to apply LoRA. Training all VLM parameters instead.")
        return model
...
def setup_optimizer_and_scheduler(model, cfg) -> Tuple[torch.optim.Optimizer, torch.optim.lr_scheduler._LRScheduler]:
    """set optimizer and scheduler for LoRA training"""
...
class LoRAVLATrainer(TrainerUtils):
    """Trainer for LoRA-based VLA finetuning"""
    ...
    def prepare_training(self):
        ...
        # Apply any additional freezing from config (should not freeze LoRA or action_model)
    ...
    def _init_checkpointing(self):
        ...
        if pretrained_checkpoint:
            ...
            logger.info("No pretrained checkpoint provided. Starting LoRA training from scratch.")
            self.completed_steps = 0
    ...
    def _log_training_config(self):
        ...
        if self.accelerator.is_main_process:
            ...
            lora_cfg = self.config.trainer.get("lora", {})
            if lora_cfg:
                logger.info(f"  LoRA rank = {lora_cfg.get('r', 16)}")
                logger.info(f"  LoRA alpha = {lora_cfg.get('lora_alpha', 32)}")
    ...
    def _finalize_training(self):
        ...
        if self.accelerator.is_main_process:
            final_checkpoint = os.path.join(self.config.output_dir, "final_lora_model")
            ...
            logger.info(f"Training complete. Final LoRA model saved at {final_checkpoint}")
            wandb.finish()
...
def main(cfg) -> None:
    logger.info("VLA LoRA Multi-GPU Training :: Warming Up")
