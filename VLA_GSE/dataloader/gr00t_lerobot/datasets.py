# SPDX-FileCopyrightText: Copyright NVIDIA CORPORATION & AFFILIATES. All rights reserved.
...
from VLA_GSE.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps

from VLA_GSE.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from VLA_GSE.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from VLA_GSE.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
...
class LeRobotSingleDataset(Dataset):
        ...
        # v3.0
            ...
            for file_path in file_paths:
                ...
                for index, episode in episodes_data.iterrows():
                    ...
                    # Use any available video from_timestamp column in v3 metadata.
                    # Different datasets may not use "observation.images.wrist".
                    from_ts = 0.0
                    for col in episode.index:
                        if isinstance(col, str) and col.startswith("videos/") and col.endswith("/from_timestamp"):
                            try:
                                from_ts = float(episode[col])
                            except Exception:
                                from_ts = 0.0
                            break
    ...
    def get_trajectory_data_lerobot_v3(self, trajectory_id: int) -> pd.DataFrame:
        ...
        if self.curr_traj_id == trajectory_id and self.curr_traj_data is not None:
            ...
            from_timestamp = self.trajectory_ids_to_metadata[trajectory_id].get("video/from_timestamp", 0.0)
